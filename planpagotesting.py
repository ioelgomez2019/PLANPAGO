...
def planpagogenerado(
    ...
    """
    Genera e imprime el plan de pagos con parámetros nombrados.

    Parámetros
    ──────────
    monto_desembolso       : Monto del préstamo en S/
    tasa_interes_anual     : TEA en porcentaje (ej: 62.9 = 62.9%)
    fecha_desembolso       : Fecha de desembolso  "DD/MM/YYYY" o date(...)
    num_cuotas             : Número de cuotas
    dia_fec_pago           : Día del mes de pago (tipo_periodo=1) o período en días (tipo=2)
    fecha_primera_cuota    : Fecha de la 1ra cuota  "DD/MM/YYYY" o date(...)
    tipo_periodo           : "Fecha Fija" | "Periodo Fijo" | 1 | 2  (default: "Fecha Fija")
    dias_gracia            : Días de gracia del desembolso           (default: 0)
    nro_cuotas_gracia      : Cuotas sin amortizar capital            (default: 0)
    forma_calculo_tasa     : 1=base 360 | 2=base 30                  (default: 1)
    cseguro_desgravamen    : Nombre o ID del seguro de desgravamen   (default: "SEGURO DESGRAVAMEN INDIVIDUAL")
                             Acepta: nombre exacto/parcial, ID numérico,
                             o "NINGUNO"/"NINGUNA"/"NONE"/"0" para prima=0
    cplan_seguro           : Nombre o ID del plan de seguros         (default: "NINGUNO")
                             Acepta los mismos valores que cseguro_desgravamen
    max_dias_primera_cuota : Máximo de días permitidos a la 1ra cuota (default: 30)
    ...
    # ── Resolver seguros (nombre, ID o NINGUNO) ───────────────────────
    if cseguro_desgravamen is None:
        id_seg = _ID_DESGRAV_INDIVIDUAL
    else:
        resuelto = resolver_seguro(cseguro_desgravamen)
        id_seg = resuelto if resuelto is not None else 8  # id=8 = SIN SEGURO (prima=0)

    id_plan = resolver_seguro(cplan_seguro)  # None → sin plan (suma 0)
    ...
    # ── Construir ParametrosPlanPago y generar ────────────────────────
...
if __name__ == "__main__":
    ...
    # # ── DEMO A: Parámetros nombrados ─────────────────────────────────
    # print("\n>>> DEMO A — planpagogenerado() con parámetros nombrados\n")
    # planpagogenerado(
    ...
    # ── DEMO B: NINGUNA en ambos → suma S/0.00 ───────────────────────
    print("\n>>> DEMO B — cseguro_desgravamen=NINGUNA, cplan_seguro=NINGUNA\n")
    planpagogenerado(
        ...
        cseguro_desgravamen     = "NINGUNA",
        cplan_seguro            = "NINGUNA",
        max_dias_primera_cuota  = 48,
    )

    # # ── DEMO C: Monto > 10,000 → auto-cambio a INDIVIDUAL ESPECIAL ───
    # print("\n>>> DEMO C — Monto > 10,000 → auto-cambio a INDIVIDUAL ESPECIAL\n")
    # planpagogenerado(
    #     monto_desembolso        = 12000.00,
    #     tasa_interes_anual      = 49.990,
    #     fecha_desembolso        = "29/01/2026",
    #     num_cuotas              = 12,
    #     tipo_periodo            = "Fecha Fija",
    #     dia_fec_pago            = 26,
    #     fecha_primera_cuota     = "26/02/2026",
    #     cseguro_desgravamen     = "SEGURO DESGRAVAMEN INDIVIDUAL",
    #     cplan_seguro            = "NINGUNO",
    #     max_dias_primera_cuota  = 30,
    # )

    # # ── DEMO D: IDs numéricos y date() directamente ──────────────────
    # print("\n>>> DEMO D — IDs numéricos y date() directamente\n")
    # planpagogenerado(
    ...
    #     tipo_periodo            = 1,
    #     dia_fec_pago            = 6,
    #     fecha_primera_cuota     = date(2026, 3, 6),
