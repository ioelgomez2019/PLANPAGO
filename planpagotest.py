...
║                                                                      ║
║  NUEVA FUNCIÓN: planpagogenerado(texto)                              ║
║  Acepta un bloque de texto con los parámetros en formato legible     ║
║                                                                      ║
║  REGLA ESPECIAL DESGRAVAMEN:                                         ║
║  Si id_seguro_desgravamen = INDIVIDUAL y monto > 10,000              ║
...
# ID del desgravamen individual (para la regla de monto > 10,000)
...
def obtener_seguro_por_nombre(nombre: str) -> TipoSeguro:
    """
    Busca un seguro por nombre (insensible a mayúsculas/tildes).
    Admite coincidencia parcial si es única.
    """
...
def resolver_seguro(valor) -> Optional[int]:
    """
    Recibe un int (ID) o str (nombre) y retorna el ID del seguro.
    Retorna None si el valor es None, 0, "0", "ninguno", "none", etc.
    """
    if valor is None:
        return None
    if isinstance(valor, int):
        if valor == 0:
            return None
        obtener_seguro_por_id(valor)   # valida que exista
        return valor
    if isinstance(valor, str):
        v = valor.strip()
        # Acepta: 0, vacío, ninguno/NINGUNO, none/NONE, no/NO, sin plan, sin seguro
        if v.lower() in ("0", "", "ninguno", "none", "no", "sin plan",
...
# ══════════════════════════════════════════════════════════════════════
# 3. PARSER DE TEXTO  ←  NUEVA FUNCIÓN
# ══════════════════════════════════════════════════════════════════════
...
def planpagogenerado(texto: str) -> list[CuotaPlan]:
    """
    Genera el plan de pagos a partir de un bloque de texto con parámetros.

    Formato aceptado (nombre = valor, uno por línea, en cualquier orden):
    ...
    Alias aceptados para los campos de seguros:
        cseguro_desgravamen  /  id_seguro_desgravamen  /  seguro_desgravamen
        cplan_seguro         /  id_plan_seguro         /  plan_seguro

    Los valores de seguros pueden ser:
        - El nombre exacto o parcial (entre comillas o sin ellas)
        - El ID numérico
        - 0 / "ninguno" / "none" → sin plan

    REGLA ESPECIAL (solo aplica al desgravamen INDIVIDUAL, id=9):
        Si monto_desembolso > 10,000 → se usa automáticamente
    ...
    for linea in texto.splitlines():
        ...
        clave = clave.strip().lower().lstrip("#").strip()
        valor = valor.strip().strip('"').strip("'").strip(",").strip()
        if clave:
            params_raw[clave] = valor

    # ── 2. Helper de lectura con aliases ────────────────────────────
    def get(claves: list[str], default=None) -> Optional[str]:
        for c in claves:
            if c in params_raw:
                return params_raw[c]
        return default

    def req(claves: list[str], desc: str) -> str:
        v = get(claves)
        if v is None:
            raise ValueError(f'Parámetro requerido no encontrado: {desc} (claves: {claves})')
        return v

    # ── 3. Parsear cada campo ────────────────────────────────────────
    monto            = float(req(["monto_desembolso", "monto"], "monto_desembolso"))
    tea_raw          = float(req(["tasa_interes_anual", "tea", "tasa"], "tasa_interes_anual"))
    # Si la tasa viene como 62.9 (porcentaje) la convertimos a decimal
    tasa             = tea_raw / 100.0 if tea_raw > 1 else tea_raw
    fecha_desembolso = _parsear_fecha(req(["fecha_desembolso", "fdesembolso"], "fecha_desembolso"))
    num_cuotas       = int(req(["num_cuotas", "cuotas", "ncuotas"], "num_cuotas"))
    dias_gracia      = int(get(["dias_gracia", "gracia", "diasgracia"], "0"))
    tipo_periodo     = _parsear_tipo_periodo(req(["tipo_periodo", "tipoperiodo", "periodo"], "tipo_periodo"))
    dia_fec_pago     = int(req(["dia_fec_pago", "dia_pago", "diapago", "diafecpago"], "dia_fec_pago"))
    fecha_1ra        = _parsear_fecha(req(["fecha_primera_cuota", "primera_cuota", "fprimera"], "fecha_primera_cuota"))
    nro_gracia       = int(get(["nro_cuotas_gracia", "cuotas_gracia"], "0"))
    forma_tasa       = int(get(["forma_calculo_tasa", "forma_tasa", "forma"], "1"))
    max_dias         = int(get(["max_dias_primera_cuota", "max_dias", "maxdias"], "30"))
    n_dec            = int(get(["n_dec_redon_calc_ppg", "ndec", "decimales"], "1"))

    # Seguros: acepta nombre, ID numérico, o NINGUNO/NONE/0
    seg_raw  = get(["cseguro_desgravamen", "id_seguro_desgravamen",
                    "seguro_desgravamen", "desgravamen"])
    plan_raw = get(["cplan_seguro", "id_plan_seguro", "plan_seguro", "plan"])

    # Si el campo NO existe en el texto → default INDIVIDUAL (9)
    # Si el campo existe pero vale NINGUNO/0/etc → id=8 (SIN SEGURO, prima=0, suma 0)
    if seg_raw is None:
        # Campo no proporcionado → usar INDIVIDUAL por defecto
        id_seg = _ID_DESGRAV_INDIVIDUAL
    else:
        resuelto = resolver_seguro(seg_raw)
        # resolver_seguro retorna None cuando el valor es NINGUNO/0/none/etc.
        id_seg = resuelto if resuelto is not None else 8  # id=8 = SIN SEGURO DESGRAVAMEN (prima=0)

    id_plan = resolver_seguro(plan_raw)  # None → sin plan (suma 0)

    # ── 4. REGLA ESPECIAL: INDIVIDUAL → ESPECIAL si monto > 10,000 ──
    monto_d = _d(monto)
    if id_seg == _ID_DESGRAV_INDIVIDUAL and monto_d > _MONTO_LIMITE_ESPECIAL:
        ...
        print(
            f"  [AUTO] Monto S/{monto:,.2f} > S/10,000 → "
    ...
    # ── 5. Construir ParametrosPlanPago y generar ─────────────────────
    p = ParametrosPlanPago(
        ...
        fecha_primera_cuota    = fecha_1ra,
        nro_cuotas_gracia      = nro_gracia,
        forma_calculo_tasa     = forma_tasa,
        id_seguro_desgravamen  = id_seg,
        id_plan_seguro         = id_plan,
        max_dias_primera_cuota = max_dias,
        n_dec_redon_calc_ppg   = n_dec,
    )

    imprimir_plan_pagos(generar_plan_pagos(p), p)
    return generar_plan_pagos(p)
...
def generar_plan_pagos(params: ParametrosPlanPago) -> list[CuotaPlan]:
    """
    Genera el plan de pagos (Sistema Francés).

    REGLA ESPECIAL (aplicada también aquí para uso directo):
        Si id_seguro_desgravamen = INDIVIDUAL (9) y monto > 10,000
        → se cambia automáticamente a INDIVIDUAL ESPECIAL (11).
    """
    # Regla especial también en la función directa
...
if __name__ == "__main__":
    ...
    # ── DEMO A: Usando planpagogenerado() con texto ───────────────────
    print("\n>>> DEMO A — planpagogenerado() con texto plano\n")
    planpagogenerado("""
        monto_desembolso        = 20000.00
        tasa_interes_anual      = 41.99
        fecha_desembolso        = 29/01/2026
        num_cuotas              = 18
        dias_gracia             = 0
        tipo_periodo            = Fecha Fija
        dia_fec_pago            = 18
        fecha_primera_cuota     = 18/03/2026
        cseguro_desgravamen     = NINGUNO
        cplan_seguro            = NINGUNO
        max_dias_primera_cuota  = 48
        n_dec_redon_calc_ppg    = 1
    """)
    ...
    # # # ── DEMO C: Usando IDs numéricos directamente ────────────────────
    # # print("\n>>> DEMO C — Parámetros con IDs numéricos\n")
    # # planpagogenerado("""
    # #     monto_desembolso        = 6662.00
    # #     tasa_interes_anual      = 75.91
    # #     fecha_desembolso        = 29/01/2026
    # #     num_cuotas              = 12
    # #     dias_gracia             = 6
    # #     tipo_periodo            = 1
    # #     dia_fec_pago            = 6
    # #     fecha_primera_cuota     = 06/03/2026
    # #     cseguro_desgravamen     = 9
    # #     cplan_seguro            = 5
    # #     max_dias_primera_cuota  = 36
    # #     n_dec_redon_calc_ppg    = 1
    # # """)