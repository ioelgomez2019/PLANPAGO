...
║                                                                      ║
║  REGLA ESPECIAL DESGRAVAMEN:                                         ║
║  Si cseguro_desgravamen = INDIVIDUAL y monto > 10,000                ║
║  → se usa automáticamente INDIVIDUAL ESPECIAL (id=11)                ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
def resolver_seguro(valor) -> Optional[int]:
    """Recibe int (ID) o str (nombre) y retorna ID. None / 'NINGUNO' / '0' → None."""
    if valor is None:
        return None
    ...
    if isinstance(valor, str):
        v = valor.strip()
        if v.lower() in ("0", "", "ninguno", "ninguna", "none", "no",
                         "sin plan", "sin seguro", "sin desgravamen", "0.00"):
            return None
        try:
            id_int = int(v)
...
def listar_seguros():
    ...
    for s in sorted(TABLA_SEGUROS.values(), key=lambda x: x.id_tipo_seguro):
        tipo = f"{s.n_valor}% × Saldo/100" if s.id_tipo_valor == 1 else f"S/{s.n_valor}/mes"
...
def calcular_prima_seguro(seguro: TipoSeguro, sal_cap: Decimal, dias: int) -> Decimal:
    if seguro.id_tipo_valor == 1:
        return _r2(sal_cap * seguro.n_valor * (_d(dias) / _d(30)))
...
def _parsear_fecha(s: str) -> date:
    ...
    if "/" in s:
        p = s.split("/")
        return date(int(p[2]), int(p[1]), int(p[0])) if len(p[0]) != 4 \
               else date(int(p[0]), int(p[1]), int(p[2]))
    if "-" in s:
        p = s.split("-")
        return date(int(p[0]), int(p[1]), int(p[2])) if len(p[0]) == 4 \
               else date(int(p[2]), int(p[1]), int(p[0]))
...
def _parsear_tipo_periodo(s: str) -> int:
    ...
    if s in ("2", "periodo fijo", "periodo_fijo", "periodofijo"):
        return 2
    raise ValueError(f'tipo_periodo "{s}" no reconocido.')


def calcular_dias_gracia_extra(fecha_desembolso: date, fecha_primera_cuota: date,
                                max_dias: int = 30) -> int:
...
def generar_plan_pagos(params: ParametrosPlanPago) -> list[CuotaPlan]:
    ...
    if (params.id_seguro_desgravamen == _ID_DESGRAV_INDIVIDUAL
        ...
        print(f"  [AUTO] Monto S/{params.monto_desembolso:,.2f} > S/10,000 → "
    ...
    gracia_extra = calcular_dias_gracia_extra(
        params.fecha_desembolso, params.fecha_primera_cuota,
    ...
    cronograma = (_cronograma_fecha_fija(params, gracia_extra)
                  ...
                  else _cronograma_periodo_fijo(params, gracia_extra))
    ...
    for iteracion in range(MAX_ITER):
        ...
        for idx, (fecha, dias, dias_acu_i) in enumerate(cronograma):
            ...
            prima_plan   = (calcular_prima_seguro(plan_obj, saldo, dias)
            ...
            capital  = (Decimal("0.00") if params.nro_cuotas_gracia >= num_cuota
                        else _r2(cuota_redond - interes - seg_comi))
...
def imprimir_plan_pagos(plan: list[CuotaPlan], params: ParametrosPlanPago):
    ...
    print(f"  Desgravamen: [{seg_obj.id_tipo_seguro}] {seg_obj.nombre}  (Prima={seg_obj.n_valor})")
    print(f"  Plan seguro: [{plan_obj.id_tipo_seguro}] {plan_obj.nombre}" if plan_obj
          else "  Plan seguro: Sin plan adicional")
    print("=" * W)
    hdr  = "{:>5}  {:<12}  {:>5}  {:>10}  {:>9}  {:>8}  {:>11}  {:>11}  {:>9}  {:>11}"
    rfmt = "{:>5}  {:<12}  {:>5}  {:>10}  {:>9}  {:>8}  {:>11}  {:>11}  {:>9}  {:>11}"
    print(hdr.format("Cuota","Fecha Pago","Días","Sal. Cap.","Capital","Interés",
...
# ══════════════════════════════════════════════════════════════════════
# 7. CONVERSIÓN A JSON (lista de dicts)
# ══════════════════════════════════════════════════════════════════════

def _plan_a_json(plan: list[CuotaPlan]) -> list[dict]:
    """
    Convierte lista de CuotaPlan → lista de dicts + fila TOTAL al final.
    ...
    El último elemento tiene "cuota": "TOTAL" con los acumulados.
    """
    result = []
    tot_cap = tot_int = tot_sd = tot_pp = tot_sc = tot_imp = Decimal(0)
...
def planpagogenerado(
    ...
    """
    Genera el plan de pagos y retorna lista de dicts (JSON-ready).

    Parámetros
    ──────────
    monto_desembolso       : Monto del préstamo en S/
    tasa_interes_anual     : TEA en porcentaje (ej: 62.9 = 62.9%)
    fecha_desembolso       : "DD/MM/YYYY" o date(2026,1,29)
    num_cuotas             : Número de cuotas
    dia_fec_pago           : Día del mes (Fecha Fija) o días del período (Periodo Fijo)
    fecha_primera_cuota    : "DD/MM/YYYY" o date(2026,2,20)
    tipo_periodo           : "Fecha Fija" | "Periodo Fijo" | 1 | 2   (default: "Fecha Fija")
    dias_gracia            : Días de gracia                           (default: 0)
    nro_cuotas_gracia      : Cuotas sin amortizar capital             (default: 0)
    forma_calculo_tasa     : 1=base 360 | 2=base 30                   (default: 1)
    cseguro_desgravamen    : Nombre, ID o "NINGUNO"/"NINGUNA"/"0"     (default: "SEGURO DESGRAVAMEN INDIVIDUAL")
    cplan_seguro           : Nombre, ID o "NINGUNO"/"NINGUNA"/"0"     (default: "NINGUNO")
    max_dias_primera_cuota : Máximo días 1ra cuota                    (default: 30)
    n_dec_redon_calc_ppg   : Decimales de redondeo de la cuota        (default: 1)

    REGLA ESPECIAL (solo para DESGRAVAMEN INDIVIDUAL, id=9):
        Si monto_desembolso > S/10,000 → cambia automáticamente a
        SEGURO DESGRAVAMEN INDIVIDUAL ESPECIAL (id=11, prima=0.001450)

    Retorna
    ───────
    Lista de dicts con las cuotas + último elemento TOTAL:
        jsonPlanPago[0]   → cuota 1   { "cuota":1, "fechaPago":"...", ... }
        jsonPlanPago[1]   → cuota 2
        jsonPlanPago[-2]  → última cuota
        jsonPlanPago[-1]  → TOTAL     { "cuota":"TOTAL", ... }

    Acceso a columnas:
        jsonPlanPago[i]["cuota"]       → nro cuota (int)
        jsonPlanPago[i]["fechaPago"]   → "DD/MM/YYYY"
        jsonPlanPago[i]["dias"]        → días del período
        jsonPlanPago[i]["salCap"]      → saldo capital
        jsonPlanPago[i]["capital"]     → amortización
        jsonPlanPago[i]["interes"]     → interés
        jsonPlanPago[i]["segDesgrav"]  → seguro desgravamen
        jsonPlanPago[i]["primaPlan"]   → prima plan
        jsonPlanPago[i]["segComi"]     → seg + comisión
        jsonPlanPago[i]["montoCuota"]  → monto total cuota
    """
    # Normalizar fechas
    ...
    # Normalizar tasa (62.9 → 0.629)
    tasa = tasa_interes_anual / 100.0 if tasa_interes_anual > 1 else tasa_interes_anual

    # Resolver seguros
    resuelto_seg = resolver_seguro(cseguro_desgravamen)
    id_seg  = resuelto_seg if resuelto_seg is not None else 8   # 8 = SIN DESGRAVAMEN
    id_plan = resolver_seguro(cplan_seguro)

    # Regla especial: INDIVIDUAL + monto > 10,000 → ESPECIAL
    monto_d = _d(monto_desembolso)
    if id_seg == _ID_DESGRAV_INDIVIDUAL and monto_d > _MONTO_LIMITE_ESPECIAL:
        seg_esp = obtener_seguro_por_id(_ID_DESGRAV_INDIVIDUAL_ESPECIAL)
        print(f"  [AUTO] Monto S/{monto_desembolso:,.2f} > S/10,000 → "
              f"Desgravamen cambiado a [{seg_esp.id_tipo_seguro}] "
              f"{seg_esp.nombre} (prima={seg_esp.n_valor})")
        id_seg = _ID_DESGRAV_INDIVIDUAL_ESPECIAL

    params = ParametrosPlanPago(
    ...
    plan = generar_plan_pagos(params)
    imprimir_plan_pagos(plan, params)
    return _plan_a_json(plan)


# ══════════════════════════════════════════════════════════════════════
# 9. CLASE CalculoPlanPago  (import: from calculoPlanPago import CalculoPlanPago)
# ══════════════════════════════════════════════════════════════════════

class CalculoPlanPago:
    """
    Clase estática. Uso:
        from calculoPlanPago import CalculoPlanPago

        jsonPlanPago = CalculoPlanPago.planpagogenerado(
            monto_desembolso       = 5800.00,
            tasa_interes_anual     = 62.900,
            fecha_desembolso       = "29/01/2026",
            num_cuotas             = 12,
            dia_fec_pago           = 20,
            fecha_primera_cuota    = "20/02/2026",
            tipo_periodo           = "Fecha Fija",
            cseguro_desgravamen    = "SEGURO DESGRAVAMEN INDIVIDUAL",
            cplan_seguro           = "PLAN 1: A. EDUCATIVA",
            max_dias_primera_cuota = 22,
        )
        # jsonPlanPago[i]     → cuota i+1
        # jsonPlanPago[-1]    → TOTAL
    """
    ...
    def planpagogenerado(
        ...
        """Ver docstring de planpagogenerado() — retorna list[dict] + TOTAL al final."""
...
# ══════════════════════════════════════════════════════════════════════
# 10. EJECUCIÓN DIRECTA — modifica aquí para tus pruebas
# ══════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    # ── Imprimir JSON como tabla ──────────────────────────────────────
    W = 118
    print()
    print("  JSON retornado por planpagogenerado()")
    print("=" * W)
    print(f"  {'Cuota':>5}  {'FechaPago':^12}  {'Dias':>4}  {'SalCap':>10}  "
          f"{'Capital':>9}  {'Interes':>9}  {'SegDesgrav':>10}  "
          f"{'PrimaPlan':>9}  {'SegComi':>7}  {'MontoCuota':>10}")
    print("-" * W)
    for f in jsonPlanPago[:-1]:
        print(f"  {f['cuota']:>5}  {f['fechaPago']:^12}  {f['dias']:>4}  "
              f"{f['salCap']:>10.2f}  {f['capital']:>9.2f}  {f['interes']:>9.2f}  "
              f"{f['segDesgrav']:>10.2f}  {f['primaPlan']:>9.2f}  "
              f"{f['segComi']:>7.2f}  {f['montoCuota']:>10.2f}")
    print("-" * W)
    t = jsonPlanPago[-1]
    print(f"  {'TOTAL':>5}  {'':^12}  {'':>4}  {'':>10}  "
          f"{t['capital']:>9.2f}  {t['interes']:>9.2f}  "
          f"{t['segDesgrav']:>10.2f}  {t['primaPlan']:>9.2f}  "
          f"{t['segComi']:>7.2f}  {t['montoCuota']:>10.2f}")
    print("=" * W)