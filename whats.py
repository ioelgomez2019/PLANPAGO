...
# ══════════════════════════════════════════════════════════════════════
# 8. CONVERSIÓN A JSON  ← ÚNICO AGREGADO
# ══════════════════════════════════════════════════════════════════════

def _plan_a_json(plan: list[CuotaPlan]) -> list[dict]:
  """
  Convierte list[CuotaPlan] → list[dict] + fila TOTAL al final.
...
# ══════════════════════════════════════════════════════════════════════
# 9. planpagogenerado  ← TU CÓDIGO ORIGINAL, solo cambia return
# ══════════════════════════════════════════════════════════════════════
...
def planpagogenerado(
    ...
    max_dias_primera_cuota:  int         = 30,
    n_dec_redon_calc_ppg:    int         = 1,
    cuotasgracia:            int         = 0,   # reservado, aún no afecta la lógica
) -> list[dict]:
  # Normalizar fechas
  ...
  # Normalizar tasa (62.9 → 0.629, ya 0.629 → se deja igual)
  ...
  return _plan_a_json(plan)   # ← único cambio respecto a tu original
...
class CalculoPlanPago:
    ...
    def imprimirJson(dataPlanPago: dict):
      """
      Genera el plan de pagos e imprime el JSON completo.
      Útil para revisar los datos antes de validar con el grid en TestComplete.

      Ejemplo de uso:
          CalculoPlanPago.imprimirJson(dataPlanPago)
      """
      import json
...
# ══════════════════════════════════════════════════════════════════════
# 11. EJECUCIÓN DIRECTA — modifica aquí para tus pruebas
# ══════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
  ...
  jsonPlanPago = planpagogenerado(
      ...
      tipo_periodo            = "Fecha Fija",
      dia_fec_pago            = 24,
      fecha_primera_cuota     = "24/02/2026",
      cseguro_desgravamen     = "NINGUNA",
      cplan_seguro            = "NINGUNA",
      max_dias_primera_cuota  = 26,
      cuotasgracia            = 0, #tambien hay dias de gracia que ya lo calcula internamente
  )
