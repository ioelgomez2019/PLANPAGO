...
def calcular_prima_seguro(
    ...
    idTipoValor=1 (tasa sobre saldo) → PROPORCIONAL a días:
        prima = round(sal_cap × nValor × dias/30, 2)
        Ej: desgravamen. Varía por saldo y días del período.

    idTipoValor=2 (prima fija S/mes) → MONTO FIJO, igual en TODAS las cuotas:
        prima = round(nValor, 2)
        Ej: PLAN 1=S/2.50, PLAN 2=S/5.75, PLAN 3=S/8.50.
        Siempre el mismo importe sin importar los días.
    """
    if seguro.id_tipo_valor == 1:
        factor = _d(dias) / _d(30)
        return _r2(sal_cap * seguro.n_valor * factor)
    elif seguro.id_tipo_valor == 2:
        return _r2(seguro.n_valor)   # fijo, sin proporcional a días
    return Decimal("0.00")
...
def generar_plan_pagos(params: ParametrosPlanPago) -> list[CuotaPlan]:
    ...
    print(f"el seguro es: {seg_desgrav_obj}")
    plan_obj = (
    ...
    for iteracion in range(MAX_ITER):
        ...
        for idx, (fecha, dias, dias_acu_i) in enumerate(cronograma):
            ...
            print(f"el seguro recuperado es: {seg_desgrav}")
            prima_plan  = (
            ...
            # también los días de gracia proporcionales: prima/30 × dias_gracia
...
if __name__ == "__main__":
    ...
    p1 = ParametrosPlanPago(
        ...
        tipo_periodo            = 1,
        dia_fec_pago            = 22,
        fecha_primera_cuota     = date(2024, 9, 22),   # 28 días → OK, sin gracia extra
        id_seguro_desgravamen   = 9,     # DESGRAVAMEN INDIVIDUAL
        id_plan_seguro          = None,  # sin plan
        max_dias_primera_cuota  = 31,    # si 1ra cuota > 30 días → suma gracia automática
        n_dec_redon_calc_ppg    = 1,     # nDecRedonCalcPpg = 1 (igual que C#)
    )
    imprimir_plan_pagos(generar_plan_pagos(p1), p1)

    # # ── DEMO 2: Con PLAN 1 ───────────────────────────────────────────
    # print("\n>>> DEMO 2 — Desgravamen individual + PLAN 1: A. EDUCATIVA (+S/2.50/mes)\n")
    ...
    #     id_plan_seguro          = 5,     # PLAN 1: A. EDUCATIVA
    ...
    # # ── DEMO 5: 1ra cuota supera 30 días → días de gracia automáticos
    # print("\n>>> DEMO 5 — 1ra cuota supera max_dias_primera_cuota (gracia automática)\n")
    # print("    Desembolso: 01/01/2026  |  1ra cuota: 24/02/2026  =  54 días > 30")
    # print("    → Se aplicarán días de gracia extra automáticamente\n")
    # p5 = ParametrosPlanPago(
    #     monto_desembolso        = 10000.00,
    #     tasa_interes_anual      = 49.99 / 100.0,
    #     fecha_desembolso        = date(2026, 1, 1),    # desembolso 01-ene
    #     num_cuotas              = 12,
    #     dias_gracia             = 0,
    #     tipo_periodo            = 1,
    #     dia_fec_pago            = 26,
    #     fecha_primera_cuota     = date(2026, 2, 26),   # 56 días → supera 30 → gracia=26
    #     id_seguro_desgravamen   = 9,
    #     id_plan_seguro          = 5,
    #     max_dias_primera_cuota  = 30,    # ← umbral: si 1ra cuota > 30 días, suma gracia
    #     n_dec_redon_calc_ppg    = 1,
    # )
    # imprimir_plan_pagos(generar_plan_pagos(p5), p5)

    # # ── DEMO 6: Consulta por ID ──────────────────────────────────────
